			 ...
			 bool 					        ipr)
{
   ...
   for (int i=0;i<hilbert;i++) 
   { 
	cout<<"Printing eigenvector number "<<i<<endl;
	//cout<<boost::format("%4i") %m<<"  "<<boost::format("%4i") %n<<"  "<<boost::format("%+5.10f") % Ix_up[nst]<<" "<<boost::format("%+5.10f") %Ix_dn[nst]<<"   "<<boost::format("%+5.10f") %Iy_up[nst]<<"   "<<boost::format("%+5.10f") %Iy_dn[nst]<<endl;
	for (int j=0;j<hilbert;j++) cout<<boost::format("%+5.10f") %eigenvecs(j,i)<<endl;
	cout<<"FINISHED Printing eigenvector number "<<i<<endl;
   	cout<<endl;
   	cout<<endl;
   }
   time(&end);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
void lanczos_spin_hole_requested_sector_with_one_rdms
		     (Ham &h, Simulation_Params &smp, int nup_spins,
		      int nup_holes, int ndn_holes, std::vector<double> &eigs)
{
   ...
   int hilbert=spin_dets.size()*uphole_dets.size()*dnhole_dets.size();
   iterations=min(iterations,hilbert);
   int how_many_eigenvecs=min(smp.how_many_eigenvecs,hilbert);
   ...
   std::vector<RMatrix> one_rdm_up,one_rdm_dn;
   cout<<"Computing 1-RDM of the up electrons....."<<endl;
   compute_one_rdm_up_electrons_alleigvecs(h.num_sites,eigenvecs, spin_dets,uphole_dets,dnhole_dets,
			   inverse_map_spin,inverse_map_uphole,
			   inverse_map_dnhole,one_rdm_up);
	  
   cout<<"Computing 1-RDM of the down electrons....."<<endl;
   compute_one_rdm_down_electrons_alleigvecs(h.num_sites,eigenvecs, spin_dets,uphole_dets,dnhole_dets,
			   inverse_map_spin,inverse_map_uphole,
			   inverse_map_dnhole,one_rdm_dn);
   
    bool        found;
    std::string str_ret; 
    std::vector< std::vector<int> > plusxpairs;
    std::vector< std::vector<int> > plusypairs;

    search_for("plusxpairs",smp.filename,str_ret,found);
    if (found) {if (str_ret.substr(0,1)==string("[")) plusxpairs=convert_string_to_vec_of_vec(str_ret);}
    
    search_for("plusypairs",smp.filename,str_ret,found);
    if (found) {if (str_ret.substr(0,1)==string("[")) plusypairs=convert_string_to_vec_of_vec(str_ret);}
     

   std::vector<double> Ix_up(how_many_eigenvecs*how_many_eigenvecs); 
   std::vector<double> Ix_dn(how_many_eigenvecs*how_many_eigenvecs); 
   std::vector<double> Iy_up(how_many_eigenvecs*how_many_eigenvecs); 
   std::vector<double> Iy_dn(how_many_eigenvecs*how_many_eigenvecs); 

   #pragma omp parallel for
   for (int nst=0;nst<how_many_eigenvecs*how_many_eigenvecs;nst++)
   {
	   	Ix_up[nst]=0.0;
	   	Ix_dn[nst]=0.0;
	   	Iy_up[nst]=0.0;
   ...
   for (int nst=0;nst<how_many_eigenvecs*how_many_eigenvecs;nst++)
   {
	   for (int p=0;p<plusxpairs.size();p++)
	   {
		int site1=plusxpairs[p][0];
		int site2=plusxpairs[p][1];
	   	Ix_up[nst]+=(one_rdm_up[nst](site1,site2)-one_rdm_up[nst](site2,site1));
	   	Ix_dn[nst]+=(one_rdm_dn[nst](site1,site2)-one_rdm_dn[nst](site2,site1));
	   }
	   for (int p=0;p<plusypairs.size();p++)
	   {
		int site1=plusypairs[p][0];
		int site2=plusypairs[p][1];
	   	Iy_up[nst]+=(one_rdm_up[nst](site1,site2)-one_rdm_up[nst](site2,site1));
	   	Iy_dn[nst]+=(one_rdm_dn[nst](site1,site2)-one_rdm_dn[nst](site2,site1));
	   }
   }
 
	  
   cout<<"======================================================================"<<endl;
   cout<<"  Displaying m,n, Ix_up, Ix_down, Iy_up, Iy_down"<<endl;
   cout<<"======================================================================"<<endl;
   for (int m=0;m<how_many_eigenvecs;m++)
   {
	 for (int n=0;n<how_many_eigenvecs;n++)
	 {
	   int nst=(m*how_many_eigenvecs)+n;
	   if (abs(Ix_up[nst])>1.0e-10 or abs(Ix_dn[nst])>1.0e-10 or abs(Iy_up[nst])>1.0e-10 or abs(Iy_dn[nst])>1.0e-10)
           {
	   	cout<<boost::format("%4i") %m<<"  "<<boost::format("%4i") %n<<"  "<<boost::format("%+5.10f") % Ix_up[nst]<<" "<<boost::format("%+5.10f") %Ix_dn[nst]<<"   "<<boost::format("%+5.10f") %Iy_up[nst]<<"   "<<boost::format("%+5.10f") %Iy_dn[nst]<<endl;
   ...
   for (int m=0;m<how_many_eigenvecs;m++)
   {
	 ...
	 {
	   int nst=(m*how_many_eigenvecs)+n;
	   for (int s1=0;s1<h.num_sites;s1++)
	   {
		for (int s2=0;s2<h.num_sites;s2++)
		{
		cout<<boost::format("%4i") %m<<"  "<<boost::format("%4i") %n<<"  "<<boost::format("%3i") % s1<<" "<<boost::format("%3i") %s2<<"   "<<boost::format("%+5.10f") %one_rdm_up[nst](s1,s2)<<"   "<<boost::format("%+5.10f") %one_rdm_dn[nst](s1,s2)<<endl;
		}
	   }
	 }
    }*/ 
