...
/////////////////////////////////////////////////////////////////////////////////////////
void compute_one_rdm_up_electrons_alleigvecs(int num_sites,
                   std::vector< std::vector<double> > const &eigenvecs,
   	 	   std::vector<int> const &spin_dets,
   	 	   std::vector<int> const &uphole_dets,
   	 	   std::vector<int> const &dnhole_dets,
   	 	   std::vector<int> const &inverse_map_spin,
		   ...
		   std::vector<RMatrix> &one_rdm)
{
      ...
      int nvecs=eigenvecs.size();

      // Initialize 1-RDMs
      one_rdm.clear();
      for (int cind=0;cind<nvecs*nvecs;cind++)
      {
	one_rdm.push_back(empty);
      	one_rdm[cind].resize(num_sites,num_sites);
      	for (int m=0;m<num_pairs;m++) one_rdm[cind][m]=0.0;
      }

      // Density matrix elements <m | c^dagger_site1 c site 2 |n >
      // where m and n are eigenvectors
      // Rewrite it as sum_j <m | c^dag 1 c 2 | j > < j |n> 
      // c dag 1 c2 |j> = sign (j) |loc(j)> ... loc is a unique configuration in Hilbert space
      // sum_j <m|loc (j)> <j|n> * sign(j)
      for (int j=0;j<hilbert;j++)
      {
	    std::vector<int> vec=convert_ind_to_vec(j,num_states);
	    int i0=vec[0];
	    ...
	    {
		    int a=btest(uphole_det,site1); 
		    for (int site2=0;site2<num_sites;site2++)
		    {
			    int b=btest(uphole_det,site2); 
			    if (site1!=site2)
			    {
				    int su=1;
				    int minsite=min(site1,site2);
				    int maxsite=max(site1,site2);
				    if (abs(upsigns[maxsite]-upsigns[minsite+1])%2 !=0 ) su=-1;
				    if (a==0 and b==1)
				    {
					int new_uphole_det=ibset(uphole_det,site1);
					new_uphole_det=ibclr(new_uphole_det,site2);
					int loc=(i0*nud)+(inverse_map_uphole[new_uphole_det]*(nd))+i2;
					...
					for (int m=0;m<nvecs;m++)
					{
						...
						{
							int cind=(m*nvecs)+n;
							one_rdm[cind](site1,site2)+=(su*(eigenvecs[m][loc]*eigenvecs[n][j]));
						}
					}
				    }
			    }
				...
				if (a==1)
				{
					...
					for (int m=0;m<nvecs;m++)
					{
						...
						{
							int cind=(m*nvecs)+n;
							one_rdm[cind](site1,site1)+=(eigenvecs[m][j]*eigenvecs[n][j]);
						}
					}
				}
			    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
void compute_one_rdm_down_electrons_alleigvecs(int num_sites,
                   std::vector< std::vector<double> > const &eigenvecs,
   	 	   std::vector<int> const &spin_dets,
   	 	   std::vector<int> const &uphole_dets,
   	 	   std::vector<int> const &dnhole_dets,
		   ...
		   std::vector<RMatrix> &one_rdm)
{
	...
	one_rdm.push_back(empty);
      ...
      for (int j=0;j<hilbert;j++)
      {
	    ...
	    for (int site1=0;site1<num_sites;site1++)
	    {
		    int a=btest(dnhole_det,site1); 
		    for (int site2=0;site2<num_sites;site2++)
		    {
			    int b=btest(dnhole_det,site2); 
			    if (site1!=site2)
			    {
				    int minsite=min(site1,site2);
				    int maxsite=max(site1,site2);
				    int sd=1;
				    if (abs(dnsigns[maxsite]-dnsigns[minsite+1])%2 !=0 ) sd=-1;
				    if (a==0 and b==1)
				    {
					int new_dnhole_det=ibset(dnhole_det,site1);
					new_dnhole_det=ibclr(new_dnhole_det,site2);
					int loc=(i0*nud)+(i1*nd)+inverse_map_dnhole[new_dnhole_det];
					#pragma omp parallel for
					for (int m=0;m<nvecs;m++)
					{
						...
						{
							int cind=(m*nvecs)+n;
							one_rdm[cind](site1,site2)+=(sd*(eigenvecs[m][loc]*eigenvecs[n][j]));
						}
					}
				    }
			    }
